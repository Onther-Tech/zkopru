...
export default class TransferEthL2 extends App {
  static code = AppMenu.TRANSFER_ETH_L2
  ...
  async run(context: Context): Promise<{ context: Context; next: number }> {
    ...
    do {
      ...
      const { address } = await this.ask({
        ...
        message: 'Send to? (Address on the other L2)',
      })
      try {
      ...
      const { address2 } = await this.ask({
        type: 'text',
        name: 'address2',
        initial: '0xabcdef0123456789abcdef0123456789abcdef012',
        message: 'Which layer2? (Address of the other L2 contract on L1)',
      })
      try {
        targetLayer2 = Address.from(address2)
      } catch (err) {
        logger.error(`Failed to get address from`)
        logger.error(err)
      }
        ...
        tx = txBuilder
          ...
          .sendEther({
            ...
            layer2: {
              ...
              layer2: Field.from(targetLayer2.toString()),
